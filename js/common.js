              ...
              if(response == 0){  // смотрим ответ от сервера и выполняем соответствующее действие
                 alert("Ошибка обработки запроса");
								...
								$("#show_more").hide();
								 ...
								 //var test = response.entities[0].cost;
                    ...
                    '<div class="img-item">'+
                        ...
                        if(response.entities[i].discountCost != null)
                         ...
                         if (response.entities[i].new !=0)
                            li_str+='<span class="item-new">New</span></div>';
                        
                    
                    li_str+= '<h3>'+response.entities[i].title+'</h3>';
                    li_str+=  '<p>'+ response.entities[i].description + '</p>';
                   
                    li_str+= (response.entities[i].discountCost ? "<span class='price'>$" + response.entities[i].discountCost+"</span>" : "<span class='price-cost'>$" + response.entities[i].cost+"</span>"); 
                    ...
                    li_str+='</li>';
								 ...
								 if(global_li_str =='' && show_el){
									//$(".catalog-list").append(li_str).show('slow');
									 ...
									 //$(".catalog-list").append(global_li_str).show('slow');
